...
class VehicleProvider: ContentProvider() {
    ...
    private lateinit var vehicleDb: VehicleDataBase
    ...
    override fun onCreate(): Boolean {
        vehicleDb = VehicleDataBase.getInstance(context!!)!!
    ...
    override fun query(
        ...
        return when(vehicleMatcher.match(uri)){
            VEHICLE_ITEM,VEHICLE_DIR -> {
                ...
                val dao = VehicleDataBase.getInstance(context!!)!!.vehicleDao()
                val cursor=
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        return when(vehicleMatcher.match(uri)){
            VEHICLE_DIR -> {
                ...
                val id = context?.let { VehicleDataBase.getInstance(it)?.vehicleDao()?.insertVehicle(vehicle) }
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        return when(vehicleMatcher.match(uri)){
            ...
            VEHICLE_ITEM -> {
                ...
                val count = VehicleDataBase.getInstance(context!!)?.vehicleDao()?.removeVehicleById(id.toInt())
                return count!!
            }
    ...
    override fun update(uri: Uri, values: ContentValues?, selection: String?, selectionArgs: Array<out String>?): Int {
        return when(vehicleMatcher.match(uri)){
            ...
            VEHICLE_ITEM -> {
                ...
                return VehicleDataBase.getInstance(context!!)?.vehicleDao()?.updateVehicle(vehicle)!!
